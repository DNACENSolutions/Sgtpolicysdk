...
Run this job from parent directory:
    pyats run job tests/job/all_features/job_1000_policytest_update.py
...
def main():
    ...
    job_list=[]
    script_name = os.path.join(BASE_RUN_PATH,'tests/testscripts/policytest/policytest_update.py')

    task1 = Task(testscript = script_name,
                    cluster_inputs=clusterinput,
                    test_inputs=testinputs,
                    taskid = 'Policytest-{}'.format(script_name.split('/')[-1]))
