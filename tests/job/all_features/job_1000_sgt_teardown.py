...
Run this job from parent directory:
    pyats run job tests/job/all_features/job_1000_sgt_teardown.py
...
def main():
    ...
    job_list=[]
    script_name = os.path.join(BASE_RUN_PATH,'tests/testscripts/sgttest/sgttest_teardown.py')

    task1 = Task(testscript = script_name,
                    cluster_inputs=clusterinput,
                    test_inputs=testinputs,
                    taskid = 'sgt_teardown-{}'.format(script_name.split('/')[-1]))
