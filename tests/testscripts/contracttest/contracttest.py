...
class Test_all_access_contract(aetest.Testcase):
    @aetest.test
    def test1_create_accesscontracts(self, steps, dnac_obj):
        logging.info("Creating Access Contracts from the input")
        for ac_data in dnac_obj.testinput["CONTRACTINPUTS"]["CREATECONTRACTLIST"]:
            with steps.start("Creating Access Contract {}".format\
                              (ac_data["contract_name"]),continue_= True) as step:
                if not dnac_obj.accesscontracts.createNewContract(ac_data\
                         ["contract_name"],description=ac_data["description"],\
                            contract_data = (ac_data["contract_data"]))['status']:
                    step.failed("Failed creating Contract:{}".format(ac_data\
                                                               ["contract_name"]))
                else:
                    step.passed("Created Access Contract: {}".format(ac_data\
                                                               ["contract_name"]))
        logging.info("Deploy after Test 1 create contract")
        dnac_obj.accesscontracts.put_acaControllerServiceDeploy()
                                
    @aetest.test
    def test2_update_accesscontracts_des(self,steps, dnac_obj):
        ...
        for ac_data in dnac_obj.testinput["CONTRACTINPUTS"]["UPDATECONTRACTLISTDES"]:
            with steps.start("Updating Access Contract {} description".format\
                                 (ac_data["contract_name"]),continue_= True) as step:
                if not dnac_obj.accesscontracts.updateAccessContract(ac_data\
                         ["contract_name"],description=ac_data["description"])['status']:
                    step.failed("Failed Updating Contract description:{}".format\
                                                          (ac_data["contract_name"]))
                else:
                    step.passed("Updated Access Contract description: {}".format\
                                                          (ac_data["contract_name"]))       
        logging.info("Deploy after Test 2 update contract description")
        dnac_obj.accesscontracts.put_acaControllerServiceDeploy()

    @aetest.test
    def test3_update_accesscontracts_data(self, steps, dnac_obj):
        ...
        for ac_data in dnac_obj.testinput["CONTRACTINPUTS"]["UPDATECONTRACTLISTDATA"]:
            with steps.start("Updating Access Contract {} data".format(ac_data\
                                          ["contract_name"]),continue_= True) as step:
                if not dnac_obj.accesscontracts.updateAccessContract(ac_data\
                         ["contract_name"],contract_data=(ac_data["contract_data"]))\
                                                                           ['status']:
                    step.failed("Failed Updating Contract data:{}".format(ac_data\
                                                                  ["contract_name"],))
                else:
                    step.passed("Updated Access Contract data: {}".format(ac_data\
                                                                   ["contract_name"]))       
        logging.info("Deploy after Test 3 update contract data")
        dnac_obj.accesscontracts.put_acaControllerServiceDeploy()

    @aetest.test
    def test4_get_accesscontracts_byname(self, steps, dnac_obj):
        logging.info("Get all Access contract details")
        for ac_data in dnac_obj.testinput["CONTRACTINPUTS"]["GETCONTRACTLIST"]:
            with steps.start("Getting Access Contract {} data".format(ac_data\
                                   ["contract_name"]),continue_= True) as step:
                if not dnac_obj.accesscontracts.get_contractAccessByName(ac_data\
                                                        ["contract_name"])['status']:
                    step.failed("Failed getting Contract data:{}".format(ac_data\
                                                                  ["contract_name"]))
                else:
                    step.passed("Got Access Contract data: {}".format(ac_data\
                                                                  ["contract_name"])) 
                    logging.info(dnac_obj.accesscontracts.get_contractAccessByName\
                                                          (ac_data["contract_name"]))
    ...
    def test6_check_contract_exist_in_dnac(self, dnac_obj):
        ...
        result = dnac_obj.accesscontracts.verifyContractExistInDnac(dnac_obj.testinput\
                                              ["CONTRACTINPUTS"]["CONTRACTCHECKLIST"])
        logging.info(result)
        if not result:
            assert False
