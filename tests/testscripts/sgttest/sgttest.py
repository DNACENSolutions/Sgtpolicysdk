...
class Test_all_sgts(aetest.Testcase):
    @aetest.test
    def test1_create_sgts(self, steps, dnac_obj):
        logging.info("Creating all SGTs from the input")
        for sgtitem in dnac_obj.testinput["SGTINPUTS"]["CREATESGTLIST"]:
            with steps.start("Creating SGT {}".format(sgtitem["sgName"]),\
                                                           continue_= True) as step:
                if not dnac_obj.securitygroups.createSecurityGroup(sgtitem["sgName"]\
                                                        ,sgtitem["sgTag"])['status']:
                    step.failed("Failed creating sgt:{}".format(sgtitem['sgName']))
                else: 
                    step.passed("Created SGT: {}".format(sgtitem['sgName']))     
         
    @aetest.test
    def test2_create_sgts_with_vn(self, steps, dnac_obj):
        logging.info("Creating all SGTs from the input")
        for sgtitem in dnac_obj.testinput["SGTINPUTS"]["CREATESGTVNLIST"]:
            with steps.start("Creating SGT {} with VN input".format(sgtitem['sgName'])\
                                                             ,continue_= True) as step:
                if not dnac_obj.securitygroups.createSecurityGroup(sgtitem["sgName"],
                            sgtitem["sgTag"],virtualNetworks=(sgtitem["virtualNetworks"]\
                                                                            ))['status']:
                    step.failed("Failed creating sgt:{}".format(sgtitem['sgName']))
                else:
                    step.passed("Created SGT: {}".format(sgtitem['sgName']))
        logging.info("Deploy after Test 1 and Test 2 create sgt")
        dnac_obj.securitygroups.pushAndVerifySecurityGroups()

    @aetest.test
    def test3_get_sgts_groupid(self, steps, dnac_obj):
        logging.info("Get all SGTs group ID details from the input")
        for sgtitem in dnac_obj.testinput["SGTINPUTS"]["GETSGTLIST"]:
            with steps.start("Getting SGT group ID for {}".format(sgtitem["sgName"])\
                                                          ,continue_= True) as step:
                if not dnac_obj.securitygroups.getSecurityGroupIdByName(sgtitem\
                                                                ["sgName"])['status']:
                    step.failed("Failed getting sgt group ID details:{}".format\
                                                                  (sgtitem["sgName"]))
                else:
                    step.passed("Got SGT group ID details: {}".format(sgtitem['sgName']))
                    logging.info(dnac_obj.securitygroups.getSecurityGroupIdByName\
                                                                  (sgtitem["sgName"]))

    @aetest.test
    def test4_get_sgts_grouptag(self, steps, dnac_obj):
        logging.info("Get all SGTs group tag details from the input")
        for sgtitem in dnac_obj.testinput["SGTINPUTS"]["GETSGTLIST"]:
            with steps.start("Getting SGT group by tag for {}".format(sgtitem["sgName"])\
                                                              ,continue_= True) as step:
                if not dnac_obj.securitygroups.getSecurityGroupIdByName(sgtitem\
                                                               ["sgName"])['status']:
                    step.failed("Failed getting sgt group tag details:{}".format\
                                                                 (sgtitem["sgName"]))
                else:
                    step.passed("Got SGT group tag details: {}".format(sgtitem['sgName']))
                    logging.info(dnac_obj.securitygroups.getSecurityGroupTagByName\
                                                                      (sgtitem["sgName"]))
    ...
    def test6_check_sgts_exist_in_dnac(self, dnac_obj):
        ...
        result = dnac_obj.securitygroups.checkSecurityGroupsExistingInDnac((dnac_obj.\
                                             testinput["SGTINPUTS"]["SECURITYGROUPLIST"]))
        logging.info(result)
        if not result['status']:
            assert False

    @aetest.test
    def test7_update_virtualnetworks_sgts(self, steps, dnac_obj):
        logging.info("Update all SGTs Virtual networks from the input")
        for sgtitem in dnac_obj.testinput["SGTINPUTS"]["UPDATESGTVNLIST"]:
            with steps.start("Updating SGT {} Virtual Networks".format(sgtitem['sgName'])\
                                                                ,continue_= True) as step:
                if not dnac_obj.securitygroups.updateSecurityGroup(sgtitem["sgName"],
                    ...
                    step.passed("Updated SGT VN : {}".format(sgtitem['sgName']))
        logging.info("Deploy after Test 7 update sgt VN")
        dnac_obj.securitygroups.pushAndVerifySecurityGroups()
                 
    @aetest.test
    def test8_update_sgts_tag(self, steps, dnac_obj):
        logging.info("Update all SGTs tag from the input")
        for sgtitem in dnac_obj.testinput["SGTINPUTS"]["UPDATESGTTAGLIST"]:
            with steps.start("Updating SGT {} Tag".format(sgtitem['sgName']),\
                                                                 continue_= True) as step:
                if not dnac_obj.securitygroups.updateSecurityGroup(sgtitem["sgName"]\
                                                             ,sgtitem["sgTag"])['status']:
                    step.failed("Failed creating sgt tag: {}".format(sgtitem["sgName"]))
                else:
                    step.passed("Updated SGT Tag: {}".format(sgtitem['sgName']))
        logging.info("Deploy after Test 8 update sgt tag")
        dnac_obj.securitygroups.pushAndVerifySecurityGroups()

    @aetest.test
    def test9_update_sgts_description(self, steps, dnac_obj):
        logging.info("Update all SGTs description from the input")
        for sgtitem in dnac_obj.testinput["SGTINPUTS"]["UPDATESGTDESLIST"]:
            with steps.start("Updating SGT {} description".format(sgtitem['sgName']),\
                                                                 continue_= True) as step:
                if not dnac_obj.securitygroups.updateSecurityGroup(sgtitem["sgName"],\
                                            description=sgtitem["description"])['status']:
                    step.failed("Failed creating sgt description: {}".format\
                                                                      (sgtitem["sgName"]))
                else:
                    step.passed("Updated SGT description: {}".format(sgtitem['sgName']))
        logging.info("Deploy after Test 9 update sgt description")
        dnac_obj.securitygroups.pushAndVerifySecurityGroups()
